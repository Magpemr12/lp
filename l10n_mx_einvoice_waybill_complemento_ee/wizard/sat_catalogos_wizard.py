...
class SAT_CatalogosWizard(models.TransientModel):
    ...
    def open_catalog(self):
        ...
        if 'waybill' in self.catalogo:
            ...
            action = imd.xmlid_to_object('l10n_mx_einvoice_waybill_base.' + self.catalogo)
            list_view_id = imd.xmlid_to_res_id('l10n_mx_einvoice_waybill_base.' + data[self.catalogo]['tree'])
            form_view_id = imd.xmlid_to_res_id('l10n_mx_einvoice_waybill_base.' + data[self.catalogo]['form'])
            ...
            imd = self.env['ir.model.data']
            action = imd.xmlid_to_object('l10n_mx_einvoice_waybill_base_address_data.' + self.catalogo)
            list_view_id = imd.xmlid_to_res_id('l10n_mx_einvoice_waybill_base_address_data.' + data[self.catalogo]['tree'])
            form_view_id = imd.xmlid_to_res_id('l10n_mx_einvoice_waybill_base_address_data.' + data[self.catalogo]['form'])
