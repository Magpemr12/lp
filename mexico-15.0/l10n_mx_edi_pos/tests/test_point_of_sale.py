...
class PointOfSale(TransactionCase):
    ...
    def base_test(self, pac_name):
        ...
        self.assertEqual(
            len(self.att_obj.search([
                ...
                ('name', 'like', '%.xml')]).ids), 1, 'Attachments not generated')
    ...
    def test_partial_refund(self):
        ...
        with order_form.lines.edit(1) as line_form:
        ...
        self.assertEqual(self.session.l10n_mx_edi_pac_status, "signed", self.session.message_ids.mapped('body'))
        self.assertEqual(
            len(self.att_obj.search([('res_model', '=', 'pos.session'), ('res_id', '=', self.session.id),
                                     ('name', 'like', '%.xml')]).ids), 2, 'Attachments not generated')
    ...
    def test_refund_in0(self):
        ...
        self.session.action_pos_session_closing_control()
        self.assertEqual(self.session.l10n_mx_edi_pac_status, "signed", self.session.message_ids.mapped('body'))
        attach = self.att_obj.search([('res_model', '=', 'pos.session'), ('res_id', '=', self.session.id),
                                      ('name', 'like', '%.xml')]).datas
