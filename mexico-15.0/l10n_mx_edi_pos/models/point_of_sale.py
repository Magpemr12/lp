...
class PosSession(models.Model):
    ...
    def _l10n_mx_edi_solfact_cancel(self, pac_info, cfdi):
        ...
        cer_pem = base64.b64encode(certificate_id.get_pem_cer(certificate_id.content)).decode('UTF-8')
    ...
    def _l10n_mx_edi_sw_sign(self, pac_info, cfdi):
        ...
        if not token:
            self.l10n_mx_edi_log_error(_("Token could not be obtained %s") % req_e)
            return {}
        url = pac_info['url']
...
class PosOrder(models.Model):
    ...
    def action_create_invoice(self):
        ...
        for order in self.filtered(lambda ord: ord.partner_id.vat and not ord.account_move):
