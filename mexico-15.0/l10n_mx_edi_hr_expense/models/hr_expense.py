...
class HrExpenseSheet(models.Model):
    ...
    def l10n_mx_edi_create_invoice(self):
        for sheet in self:
            ...
            if not with_cfdi or all(
                    inv.payment_state in ('in_payment', 'paid') for inv in
    ...
    def action_open_invoices(self):
        return {
            ...
            'domain': [('id', 'in', self.expense_line_ids.mapped(
                'l10n_mx_edi_invoice_id').filtered(lambda inv: inv.state in ('posted', 'draft')).ids)],
    ...
    def action_open_invoices_paid(self):
        return {
            ...
            'type': 'ir.actions.act_window',
            'domain': [('id', 'in', self.expense_line_ids.mapped(
                'l10n_mx_edi_invoice_id').filtered(lambda inv: inv.payment_state in ('in_payment', 'paid')).ids)],
            'context': {'move_type': 'in_invoice',
                        'search_default_group_by_partner_id': 1},
        }
...
class HrExpense(models.Model):
        ...
        return {
            ...
            'context': {
                ...
                'default_petty_journal_id': employee.journal_ids.id if len(
                    employee.journal_ids) == 1 and todo[0].payment_mode == 'petty_account' else False,
            }
        }
    ...
    def create_partner_from_cfdi(self):
        ...
        for expense in self.filtered('l10n_mx_edi_rfc'):
            ...
            domain = [('company_id', 'in', [False, self.company_id.id]), ('vat', '=', expense.l10n_mx_edi_rfc),
                      ('is_company', '=', True)]
    ...
    @api.depends('sheet_id', 'sheet_id.account_move_id', 'sheet_id.state',
                 'l10n_mx_edi_invoice_id.state', 'l10n_mx_edi_invoice_id.payment_state')
        ...
        for expense in self:
            if not expense.sheet_id:
                ...
                expense.state = "refused"
                  ...
                  expense.l10n_mx_edi_invoice_id.payment_state not in ('in_payment', 'paid')):
                expense.state = 'approved'
            elif (expense.sheet_id.state == "done" or
                  expense.l10n_mx_edi_invoice_id.payment_state in ('in_payment', 'paid')):
                expense.state = "done"
    ...
    def _force_create_super_employee(self):
        with registry(self.env.cr.dbname).cursor() as _cr:
            ...
            employee = _env['hr.employee'].with_context(
                active_test=False).sudo().search([('user_id', '=', SUPERUSER_ID)], limit=1)
            if not employee:
    ...
    def ack(self):
        ...
        **context**:
        ...
        for expense in self:
            ...
            if not user:
                _logger.error("Impossible to send any ACK from expenses if employee incorrectly set.",)
...
3. We will try to do an action depending on the key from the mail which are.:
    ...
    'date', 'body', 'attachments', 'author_id'`
        ...
        partner_ids = [p.id for p in self.env['mail.thread']._mail_find_partner_from_emails(
            email_list, records=self, force_create=False) if p]
   ...
   <ns1:Body>
      <ns0:Consulta>
        ...
        for rec in self:
            ...
            try:
                ...
                continue
            rec.l10n_mx_edi_sat_status = CFDI_SAT_QR_STATE.get(status[0] if status else '', 'none')
        ...
        return {
            ...
            6: {
                ...
                "message_ok": _("It is inside the safety period (current "
                                "period or max 5 days at the beginning of the next)"),
            ...
            15: {
                ...
                "message": _("The journal in the employee is required for "
                             "expenses paid for the employee or with petty cash."),
            ...
            19: {
                ...
                "message": _("It looks like the CFDI have Local Taxes "
                             "complement and some of that taxes are not found."),
    ...
    def l10n_mx_edi_create_expense_invoice(self):
        ...
        for exp in expenses - to_omit:
            ...
            invoice = self.env['account.move'].sudo().with_context(**context).create(data)
    ...
    def _get_expenses_force_invoice_generation(self):
        ...
        return self.filtered(
            lambda e: e.l10n_mx_edi_functionally_approved and
            e.l10n_mx_edi_fiscally_approved and not e.l10n_mx_edi_analysis and label in e.partner_id.category_id)
    ...
    def create_extra_line_local_taxes(self, tax):
        self.ensure_one()
        ieps = self.env.ref(
            'l10n_mx_edi_hr_expense.product_product_ieps', False) if 'IEPS' in tax['tax'].upper() else False
    ...
    def l10n_mx_edi_get_invoice_data(self, inv):
        ...
        document_type = self.l10n_mx_edi_document_type
        journal = invoice.with_context(default_move_type=document_type)._get_default_journal()
        account = self.account_id.id or (journal.default_account_id.id if document_type in (
            'out_invoice', 'in_refund') else journal.default_account_id.id)
        invoice_line_ids = []
        tax_global = self._get_tax_global()
        label = self.env.ref('l10n_mx_edi_hr_expense.tag_force_invoice_total')
        for line in inv.get('lines', []):
            no_id = line['no_id']
            restrict_product_search = self.env['ir.config_parameter'].sudo().get_param(
                'l10n_mx_edi_hr_expense.restrict_product_search')
            res = False
        ...
        acc_pay_term = (
            ...
            if not acc_pay_term and inv['payment'] in ('PPD', False) else acc_pay_term)
        cfdi_origin = ''
        if inv.get('relation_type'):
            cfdi_origin = '%s|%s' % (inv['relation_type'], ','.join(inv['cfdi_related']))
        return {
            'partner_id': self.partner_id.id,
    ...
    def l10n_mx_edi_get_tax(self, tax, product_id=False):
        ...
        if tax['type'] == 'ret':
            rate = rate * -1
        domain = [('invoice_repartition_line_ids.tag_ids', 'in', tag_codes.get(tax['tax'])),
    ...
    def l10n_mx_edi_get_tax_group(self, taxes):
        ...
        tag_taxes = tax.browse(taxes).sudo().filtered(
            lambda tax: tag_tax in tax.invoice_repartition_line_ids.tag_ids.ids)
        ...
        for group in grouped_taxes:
            if set(tag_taxes.ids).issubset(group.children_tax_ids.ids):
                return (group + tax.browse(taxes).filtered(
                    lambda tax: tag_tax not in tax.invoice_repartition_line_ids.tag_ids.ids)).ids
    ...
    def l10n_mx_edi_get_cfdi(self):
        ...
        attachments = self.env['ir.attachment'].search([
            ('res_model', '=', self._name),
            ('res_id', 'in', self.ids),
        ]).filtered(lambda a: a.l10n_mx_edi_is_cfdi33() is not None)
    ...
    def action_move_create(self):
        ...
        expenses = self.filtered(lambda exp: not exp.l10n_mx_edi_analysis and
                                 label not in exp.partner_id.category_id and not exp.l10n_mx_edi_move_line_id)
    ...
    def l10n_mx_edi_get_local_tax(self, tax):
        ...
        if tax['type'] == 'ret':
            rate = rate * -1
        domain = [('invoice_repartition_line_ids.tag_ids', 'in', local.id),
                  ('type_tax_use', '=', 'purchase'),
                  ('amount', '=', rate)]
        if tax['type'] == 'ret' and -10.67 <= rate <= -10.66:
            domain[-1] = ('|')
    ...
    def l10n_mx_edi_revert_expense(self):
        ...
        for record in self.filtered('sheet_id'):
            ...
            if invoice and invoice.state != 'cancel':
                ...
                invoice.write({'ref': '%s-cancelled' % invoice.ref, })
