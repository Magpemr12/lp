...
class EdiHrExpense(TransactionCase):
    ...
    def test_create_expense(self):
        ...
        expense = self.env['hr.expense'].with_env(self.env(user=self.new_user)).create({
            'name': 'Expense demo',
    ...
    def test_create_partner_from_cfdi(self):
        ...
        self.assertTrue(expense.partner_id.vat == 'EKU9003173C9',
                        "Vat was incorrectly set on create partner from expense")
        self.assertTrue(expense.partner_id.zip == '37200',
                        "Zip code was incorrectly set on created partner from expense")
        expense.create_partner_from_cfdi()
        ...
        self.assertTrue(expense2.partner_id.vat == 'EKU9003173C9',
                        "Vat the partner was found and is correct with compute")
    ...
    def test_expense_state(self):
        ...
        statement = self.env['account.bank.statement'].sudo().with_context(edi_test_mode=True).create({
            'name': 'test_statement',
            'date': invoice.date,
        ...
        receivable_line = invoice.sudo().line_ids.search([('move_id', '=', invoice.id)]).filtered(
    ...
    def test_expense_local_taxes(self):
        ...
        self.xml_signed = misc.file_open(os.path.join(
            'l10n_mx_edi_hr_expense', 'tests', 'LocalTaxes.xml'),
            'r').read().encode('UTF-8')
    ...
    def test_expense_local_taxes_expense(self):
        ...
        expense = self.env['hr.expense'].with_env(self.env(user=self.uid)).create({
            'name': 'Expense local tax',
            'product_id': self.product.id,
            'employee_id': self.employee.id,
        })
        ...
        self.xml_signed = misc.file_open(os.path.join(
            'l10n_mx_edi_hr_expense', 'tests', 'LocalTaxes.xml'), 'r').read().encode('UTF-8')
        self.create_attachment(expense.id)
        taxes = self.env['account.tax'].sudo().search([
            ('type_tax_use', '=', 'purchase'), ('amount', '=', 16.0)], limit=1)
        taxes.write({'amount': 8.0})
        taxes.mapped('invoice_repartition_line_ids').write({'tag_ids': [(4, self.ref('l10n_mx.tag_iva'))]})
        expense.check_fiscal_status()
        expense.write({
    ...
    def test_expense_ieps(self):
        ...
        self.xml_signed = misc.file_open(os.path.join(
            'l10n_mx_edi_hr_expense', 'tests', 'IEPS.xml'), 'r').read().encode('UTF-8')
        self.create_attachment(expense.id)
        taxes = self.env['account.tax'].sudo().search([
            ('type_tax_use', '=', 'purchase'),
        ])
    ...
    def test_expense_by_email(self):
        ...
        employee.address_home_id.sudo().create({
            'name': 'Luis',
            'email': 'luis_t@vauxoo.com'
        })
